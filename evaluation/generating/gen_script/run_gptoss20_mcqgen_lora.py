...
# CONFIG
# -----------------------------
LORA_REPO = "HAJERI/gpt-oss-20b-endocrinology-scenario-mcq-it-lora"
...
def generate_mcq_raw(gen_pipe, topic: str, max_new_tokens: int = 512) -> str:
    ...
    # IMPORTANT: pipeline returns a list[{"generated_text": "..."}]
...
def parse_mcq_text(text: str) -> MCQ:
    ...
    # Correct letter: prefer "Correct option:", fall back to "Correct answer:"
    correct_letter = ""
    ...
    # If we have explicit per-option explanation lines, use them
    explanations = [
    ...
    # If all explanations are empty but there is a single "Explanation:" block,
    # try to split that into A/B/C/D based on lines beginning with "A:", "B:", etc.
    if not any(explanations) and idx_explanation_block != -1:
        # Everything after the "Explanation:" line
...
if __name__ == "__main__":
    # Ensure: huggingface-cli login (token with access to openai/gpt-oss-20b + your LoRA repo)
...
# To run:
# CUDA_VISIBLE_DEVICES=0 python run_gptoss20_mcqgen_lora.py
